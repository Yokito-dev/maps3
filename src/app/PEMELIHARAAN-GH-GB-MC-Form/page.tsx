    ...
    <div className="h-screen overflow-hidden font-poppins flex flex-col">
      ...
      <div className="px-4 pt-3 shrink-0">
        <div className="bg-white rounded-full shadow-lg px-6 py-1 flex items-center gap-3">
          ...
          <h1 className="font-medium">Pemeliharaan GH GB MC Form</h1>
        </div>
      </div>
    ...
    <>
      <div>
        ...
        <div className="relative mt-2 h-[220px] border-2 border-dashed border-[#2FA6DE] rounded-2xl flex items-center justify-center">
          {preview ? (
            <>
              <img src={preview} onClick={() => setOpen(true)} className="max-h-full cursor-pointer" />
              <button onClick={() => setFile(null)} className="absolute top-2 right-2 bg-red-500 text-white w-8 h-8 rounded-full flex items-center justify-center">
                <IoClose />
              </button>
            </>
          ) : (
      ...
      {open && preview && (
        <div onClick={() => setOpen(false)} className="fixed inset-0 bg-black/70 z-50 flex items-center justify-center">
          <img src={preview} className="max-w-[90vw] max-h-[90vh] rounded-xl" />
        </div>
      )}
    </>
  )
